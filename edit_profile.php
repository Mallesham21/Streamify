...
// Fetch user data
$user = null;
$stmt = $conn->prepare("SELECT username, email, mobile_no, profile_pic, subscription_type FROM users WHERE user_id = ?");
...
if (!empty($mobile_no)) {
    ...
    if (strlen($mobile_no) < 10) {
        ...
        $stmt = $conn->prepare("SELECT user_id FROM users WHERE mobile_no = ? AND user_id != ?");
        $stmt->bind_param('si', $mobile_no, $_SESSION['user_id']);
        $stmt->execute();
        $stmt->store_result();
        if ($stmt->num_rows > 0) {
...
if (empty($errors) && !$is_password_change) {
    $stmt = $conn->prepare("UPDATE users SET username = ?, email = ?, mobile_no = ?, profile_pic = ? WHERE user_id = ?");
    $stmt->bind_param('ssssi', $username, $email, $mobile_no, $profile_pic, $_SESSION['user_id']);
    ...
    <div class="edit-profile-container">
        ...
        <div class="edit-profile-card">
            ...
            <form method="POST" enctype="multipart/form-data" novalidate>
                ...
                <!-- Add this after the email field in edit_profile.php -->
<div class="mb-3">
    <label for="mobile_no" class="form-label">Mobile Number</label>
    <input type="tel" class="form-control <?= isset($errors['mobile_no']) ? 'is-invalid' : '' ?>" 
           id="mobile_no" name="mobile_no" value="<?= htmlspecialchars($user['mobile_no'] ?? '') ?>" 
           placeholder="Enter your mobile number">
    <?php if (isset($errors['mobile_no'])): ?>
        <div class="invalid-feedback"><?= htmlspecialchars($errors['mobile_no']) ?></div>
    <?php endif; ?>
</div>
